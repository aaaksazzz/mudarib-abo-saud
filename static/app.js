...
if(menu){let handled=0;const tap=e=>{const now=Date.now();if(now-handled<500)return;handled=now;toggleDrawer(e)};menu.addEventListener("pointerdown",tap,{passive:false});menu.addEventListener("click",e=>{if(Date.now()-handled<500){e.preventDefault();return}tap(e)})}const drawerTheme=$("#drawerTheme");drawerTheme?.addEventListener("click",()=>{$("#theme")?.click()});

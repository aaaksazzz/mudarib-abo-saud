...
// تحديث خريطة الأقسام لتشمل صفحة الفيوتشر
Function showSection(id){
    ...
    Const names={
        ...
        Futures:'صفقات الفيوتشر', // إضافة القسم الجديد
...
$('sortField').onchange=e=>{state.sort=e.target.value;renderScanner()};
$('sortDir').onclick=()=>{state.dir*=-1;$('sortDir').textContent=state.dir===-1?'↓ تنازلي':'↑ تصاعدي';renderScanner()};
...
// دالة الأخبار المحدثة للغة العربية المباشرة
Async function loadNews(){
    Const box=$('newsList');
    Box.innerHTML='<div class="empty-card">جاري تحميل الأخبار العربية...</div>';
    Try {
        Const res = await fetch('https://api.rss2json.com/v1/api.json?rss_url=https://ar.cointelegraph.com/rss');
        Const data = await res.json();
        
        If(data.status === 'ok' && data.items.length) {
            Box.innerHTML = data.items.map(n => {
                Const cleanDesc = n.description ? N.description.replace(/<[^>]*>?/gm, '').slice(0, 130) : '';
                Const dateStr = new Date(n.pubDate).toLocaleDateString('ar-SA', {day:'numeric', month:'short', hour:'2-digit', minute:'2-digit'});
                Return `<a class="news-card" href="${n.link}" target="_blank" rel="noopener">
                    <small>Cointelegraph العربي · ${dateStr}</small>
                    <h3>${n.title}</h3>
                    <p>${cleanDesc}...</p>
                </a>`;
            }).join('');
        } else {
            Throw new Error("لا توجد أخبار متاحة");
        }
    } catch(e) {
        // العودة للـ API القديم في حال تعثر السحب المباشر
        Try {
            Const d=await api('/api/news');
            Box.innerHTML=d.news?.length?d.news.map(n=>`<a class="news-card" href="${n.link}" target="_blank" rel="noopener"><small>${n.source} · ${n.published||''}</small><h3>${n.title}</h3><p>${n.description||''}</p></a>`).join(''):'<div class="empty-card">لا توجد أخبار متاحة حاليًا.</div>';
        } catch(err) {
            Box.innerHTML=`<div class="empty-card">تعذر تحميل الأخبار العربية</div>`;
        }
    }
}

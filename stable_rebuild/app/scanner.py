...
async def scan_binance(market,interval):
    symbols=(await binance_usdt_symbols(market))[:100]
...
async def scan_market(market,interval="15m",limit=70):
    ...
    rows=await (scan_binance(market,interval) if market in ("crypto","futures") else scan_yahoo(market,interval))

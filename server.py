...
BINANCE_TIMEOUT = float(
    os.getenv("BINANCE_TIMEOUT", "8")
)

BINANCE_REQUEST_DELAY = float(
    os.getenv("BINANCE_REQUEST_DELAY", "0.35")
)

MARKET_CACHE_TTL = int(
    os.getenv("MARKET_CACHE_TTL", "900")
)

TICKER_CACHE_TTL = int(
    os.getenv("TICKER_CACHE_TTL", "45")
)

SPOT_CACHE_TTL = int(
    os.getenv("SPOT_CACHE_TTL", "900")
)

SCAN_CACHE_TTL = int(
    os.getenv("SCAN_CACHE_TTL", "45")
)

SPOT_CACHE_FILE = os.getenv(
    "SPOT_CACHE_FILE",
    "spot_cache.json"
)
...
def save_spot_cache():
    try:
        with CACHE_LOCK:
            data = dict(SPOT_DATA_CACHE)

        tmp = SPOT_CACHE_FILE + ".tmp"
    ...
    for base in BINANCE_BASES:
        ...
        try:
            ...
            with CACHE_LOCK:
                ...
                elapsed = (
                    time.time()
                    - LAST_BINANCE_REQUEST
                )

            if elapsed < BINANCE_REQUEST_DELAY:

                time.sleep(
                    BINANCE_REQUEST_DELAY
                    - elapsed
                )
...
# =========================================================
# SAME ANALYSIS FOR SPOT / ALPHA / FUTURES
# =========================================================

def analysis_for_symbol(
    symbol,
    interval
):
    ...
    klines, cached = get_klines_cached(
        symbol,
        interval,
        230
    )
    ...
    with CACHE_LOCK:
        ...
        item["analysis_ts"] = time.time()

        SPOT_DATA_CACHE[key] = item
...
def futures_result(
    ...
    result["leverage"] = FUTURES_LEVERAGE

    result["leverageText"] = (
        f"{FUTURES_LEVERAGE}x"
    )
...
def futures_analysis():
    ...
    try:
        ...
        result = futures_analysis_result(
            a
        )
